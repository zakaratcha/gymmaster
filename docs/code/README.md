...
| Каталог     | Роль                                                                                                                      |
| ----------- | ------------------------------------------------------------------------------------------------------------------------- |
| `blocks/`   | Компоненты по **БЭМ** и в **React-договорённости имён**: папка **PascalCase** = файл **Block.tsx** и SCSS **Block.scss**. |
...
- Строки `className` собираются только через **`cn`** из **`@bem-react/classname`**; селекторы в **`.scss`** соответствуют тому же неймингу (`LoginForm`, `LoginForm-Main`, … для React-схемы пакета). Стили блока оформляются вложенными селекторами по БЭМ-сущностям (`&-Element`, `&_modifier`).
